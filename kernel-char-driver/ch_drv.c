...
static ssize_t my_read(struct file *f, char __user *buf, size_t len, loff_t *off)
{
	...
	for (k = 0; k < eidx && lenb < obuf_sz - 50; k++) {
		bytes = snprintf(obuf + lenb, obuf_sz - lenb, 
				"Space count for string \"%s\": %d\n", entries[k].strings, entries[k].spaces);
	...
	if (eidx == 0) {
		bytes = snprintf(obuf, obuf_sz, "No space counts recorded yet.\n");
		if (bytes > 0) {
			lenb = bytes;
		}
	}
...
static ssize_t my_write(struct file *f, const char __user *buf,  size_t len, loff_t *off)
{
  ...
  entries[eidx].spaces = cnt;
  eidx++;

  printk(KERN_INFO "Driver: Stored string with %d spaces\n", cnt);
